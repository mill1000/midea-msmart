...
class TestProtocol(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_send_exceptions(self) -> None:
        ...
        lan._connect = mock.AsyncMock()
        lan._protocol = _LanProtocol()
        lan._protocol._peer = "192.168.1.69:6444"
        
        # lan._protocol.alive = mock.MagicMock(return_value=True)
        lan._protocol._transport = mock.MagicMock()
        lan._protocol._transport.is_closing = mock.MagicMock(return_value=False)

        # with patch("asyncio.timeouts.timeout", side_effect=asyncio.TimeoutError) as mock_queue_get:
        # with patch("asyncio.Queue.get", side_effect=asyncio.TimeoutError) as mock_queue_get:
        task1 = asyncio.create_task(device.refresh()) 
        await asyncio.sleep(2)
        task2 = asyncio.create_task(device.refresh()) 
        await task1
        await task2
